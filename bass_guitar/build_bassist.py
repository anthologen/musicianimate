"""Builds a blocky, faceless, minimalist humanoid bassist to stand in for a
real character model later on.

Run inside Blender (optionally after build_bass_guitar.py) to create a
"Bassist" collection: a single ``Bassist`` armature (average adult proportions,
standing upright) with rigid box meshes on every bone and shoes on the feet.

Like the guitarist stand-in (and unlike the drummer, who sits bolted to the
kit), the bassist is a free-standing, walk-capable figure. The rig is built so
an animator can:
...
attach point for the bass animation pipeline's hand rigs (build_hands.py's
``FretHand`` / ``PluckHand`` / ``PickHand`` armatures): parent each hand armature
to the matching ``hand.*`` bone later and the arm IK will carry the hands wherever
the wrists go. The hand stub is rolled to a predictable local-Z-up frame so that
attachment is consistent on both sides.

The figure stands at the world origin, feet on the z=0 floor, facing -Y (left =
+X, right = -X, up = +Z) -- the same axis convention as the drummer/guitarist
stand-ins.
...
Usage (Blender Python console / MCP execute_blender_code)::
    ...
    spec = importlib.util.spec_from_file_location(
        "build_bassist", "/path/to/bass_guitar/build_bassist.py")
...
def _build_skeleton(coll):
    arm_data = bpy.data.armatures.new("BassistRig")
    arm = bpy.data.objects.new("Bassist", arm_data)
    ...
    for side in ("L", "R"):
        ...
        elbow = V(_solve_mid(s, wrist, ANTHRO["upper_arm"], ANTHRO["forearm"],
                             (0.4 * outx, 0.9, -0.2)))
        # Hand stub: continues down-and-forward from the wrist (where the bass
        ...
        # FretHand / PluckHand rigs attach with a consistent frame on each side.
...
def _clothe(arm, coll):
    skin = _mat("BassistBody", (0.30, 0.33, 0.38), 0.6)   # neutral grey mock
    dark = _mat("BassistDark", (0.13, 0.14, 0.17), 0.6)
    rubber = _mat("BassistShoe", (0.04, 0.04, 0.05), 0.6)
    ...
    for side in ("L", "R"):
        ...
        # bass hand rig will sit over/replace.
...
def build_bassist():
    ...
    old = bpy.data.collections.get("Bassist")
    if old is not None:
        for o in list(old.objects):
            bpy.data.objects.remove(o, do_unlink=True)
        bpy.data.collections.remove(old)

    coll = bpy.data.collections.new("Bassist")
    ...
    bpy.context.view_layer.update()
    print(f"Built bassist: armature '{arm.name}' with "
