...
Usage::
    ...
    blender --background --python guitar/scan_finger_collisions.py
"""
import os
import sys
...
def seg_dist(p1, q1, p2, q2):
    ...
    best = None
    if r1 is not None:
        # Clamp the infinite-line solution into both segments.
        def clamp(pt, a, b):
            ab = b - a
        ...
        c1 = clamp(r1, p1, q1)
        c2 = clamp(r2, p2, q2)
        # One clamping pass each way tightens the pair.
        c2 = clamp(c2, p2, q2)
...
per_frame = {}  # (fa, fb, frame) -> dist, for the hold report
...
for label, frame in (("C", 202), ("Am", 226), ("E", 250), ("F", 274),
    ...
    worst = min(((per_frame[(fa, fb, frame)], fa, fb)
                 for fa in (1, 2, 3) for fb in range(fa + 1, 5)))
    print(f"  {label:<8} frame {frame}: {worst[0]*1000:5.1f} mm "
          f"(fingers {worst[1]}+{worst[2]})")
print("SCAN DONE")
